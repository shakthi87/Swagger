...
describe('dotenv', function () {
  ...
  describe('config', function () {
    ...
    it('takes option for silencing errors', function (done) {
      ...
      errorStub.called.should.eql.false; //jshint ignore:line
      done();
    });
  });
  ...
  describe('parse', function () {
    ...
    it('should parse a buffer from a file into an object', function (done) {
      ...
      payload.BASIC.value.should.eql('basic');
      done();
    });

    it('sets basic environment variable', function (done) {
      parsed.BASIC.value.should.eql('basic');
      done();
    });

    it('reads after a skipped line', function (done) {
      parsed.AFTER_LINE.value.should.eql('after_line');
      done();
    });
    ...
    describe('expanding variables', function () {
      ...
      it('expands environment variables like $BASIC', function (done) {
        parsed.BASIC_EXPAND.value.should.eql('basic');
        done();
      });
      ...
      it('defers to process.env', function (done) {
        // from `before`
        parsed.TEST_EXPAND.value.should.eql('test');
        done();
      });

      it('defaults missing variables to an empty string', function (done) {
        parsed.UNDEFINED_EXPAND.value.should.eql('');
        done();
      });

      it('does not expand escaped variables', function (done) {
        parsed.ESCAPED_EXPAND.value.should.equal('$ESCAPED');
        done();
      });
    });

    it('defaults empty values to empty string', function (done) {
      parsed.EMPTY.value.should.eql('');
      done();
    });

    it('escapes double quoted values', function (done) {
      parsed.DOUBLE_QUOTES.value.should.eql('double_quotes');
      done();
    });

    it('escapes single quoted values', function (done) {
      parsed.SINGLE_QUOTES.value.should.eql('single_quotes');
      done();
    });

    it('expands newlines but only if double quoted', function (done) {
      parsed.EXPAND_NEWLINES.value.should.eql('expand\nnewlines');
      parsed.DONT_EXPAND_NEWLINES_1.value.should.eql('dontexpand\\nnewlines');
      parsed.DONT_EXPAND_NEWLINES_2.value.should.eql('dontexpand\\nnewlines');
      done();
    });
    ...
    it('respects equals signs in values', function (done) {
      parsed.EQUAL_SIGNS.value.should.eql('equals==');
      done();
    });

    it('retains inner quotes', function (done) {
      parsed.RETAIN_INNER_QUOTES.value.should.eql('{\"foo\": \"bar\"}');
      parsed.RETAIN_INNER_QUOTES_AS_STRING.value.should.eql('{\"foo\": \"bar\"}');
      done();
    });
    });
    ...
    describe('save', function() {
        ...
        beforeEach(function(done) {
            writeFileSyncStub = s.stub(fs, 'writeFileSync');
            done();
        });

        it('can set', function(done) {
            dotenv.set('test', 123, {path: 'test/.env'});
            done();
        });

        it('cannot set', function(done) {
            dotenv.set('test', 123, {path: 'test-not-exists/.env', silent: true});
            done();
        });
    });
});
